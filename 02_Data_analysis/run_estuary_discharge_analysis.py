...
# from FUNCTIONS.FUNCS_utils import load_data
# from FUNCTIONS.FUNCS_variability_metrics import analyze_discharge_metrics, visualize_discharge_metrics
...
# TIF_FILE = r"U:\PhDNaturalRhythmEstuaries\Data\01_Discharge_var_int_flash\Nienhuis2020_Scripts\bqart_a.tif"       # Set to path if basin area TIF file is needed, relative to INPUT_DIR
# CACHE_FILE = Path(OUTPUT_DIR) / "estuary_timeseries_cache.xlsx"  # Cache file for extracted time series
...
# estuary_coords = {
#     # 'Mississippi': (29.15, -89.25),  # USA, validation of method
#     # 'Amazon': (-0.1, -50.4),         # Brazil, validation of method
...
# #%% Check if cache exists and load, otherwise extract from .mat and save to cache
# if CACHE_FILE.exists():
#     print("Cache found. Loading extracted estuary time series from cache...")
#     cache_xl = pd.read_excel(CACHE_FILE, sheet_name=None, index_col=0, parse_dates=True)
    
#     estuary_discharge_data = {name: cache_xl[f'Q_{name}'].squeeze().values 
#                                for name in estuary_coords 
#                                if f'Q_{name}' in cache_xl}
#     estuary_sed_data       = {name: cache_xl[f'S_{name}'].squeeze().values 
#                                for name in estuary_coords 
#                                if f'S_{name}' in cache_xl}
    
#     # Recover datetimes from the index of the first sheet
#     first_sheet = next(iter(cache_xl.values()))
#     datetimes = first_sheet.index.tolist()
    
#     # Recover rm coords from the dedicated sheet
#     rm_df = cache_xl.get('rm_coords')
#     estuary_rm_coords = {row.Index: (row.rm_lon, row.rm_lat) 
#                          for row in rm_df.itertuples()} if rm_df is not None else {}
...
#     # --- Save cache ---
#     print("Saving extracted time series to cache...")
#     with pd.ExcelWriter(CACHE_FILE, engine='openpyxl') as writer:
#         for estuary in estuary_coords:
#             # Discharge sheet
#             pd.DataFrame(
...
#                 index=datetimes, 
#                 columns=['discharge_m3s']
#             ).to_excel(writer, sheet_name=f'Q_{estuary[:28]}')  # Excel sheet names max 31 chars
...
#         # Save rm coords in a dedicated sheet
#         pd.DataFrame.from_dict(
#             estuary_rm_coords, orient='index', columns=['rm_lon', 'rm_lat']
#         ).to_excel(writer, sheet_name='rm_coords')
...
# #%% Extract time series for each estuary and plot
...
#     valid = vals[~np.isnan(vals)]
#     print(f"{name}: {len(valid)} valid values, mean={np.nanmean(vals):.2f}, type={type(series)}")
