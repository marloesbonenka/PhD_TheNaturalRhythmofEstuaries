...
ESTUARIES = {
    'Western Scheldt': dict(mouth=(51.42,   3.57)),         #NL
    'Yangon':          dict(mouth=(16.52,  96.29)),         #Myanmar
    'Sittuang':        dict(mouth=(17.11,  96.94)),         #Myanmar
    'Chao Phraya':     dict(mouth=(13.55, 100.59)),         #Thailand
    'Gironde':         dict(mouth=(45.58,  -1.05)),         #France
    'Colorado (MX)':   dict(mouth=(31.83, -114.82)),        #Mexico
    'Cacipore':        dict(mouth=(3.6, -51.2)),            #French Guiana
    # 'Humber':          dict(mouth=(53.62,  -0.11)),
    # 'Fly':             dict(mouth=(-8.62, 143.70)),
    # 'Ord':             dict(mouth=(-15.5,  128.35)),
    # 'Demerara':        dict(mouth=( 6.79,  -58.18)),
    'Suriname':        dict(mouth=( 5.84,  -55.11)),        #Suriname
...
#%%
# =============================================================================
# MAIN LOOP – iterate over all active estuaries
# =============================================================================
import matplotlib.colors as mcolors
from mpl_toolkits.axes_grid1 import make_axes_locatable

yearly_col = build_yearly_collection()   # build once, reuse for all estuaries

for ACTIVE_ESTUARY, _cfg in ESTUARIES.items():
    print(f"\n{'='*60}\nProcessing: {ACTIVE_ESTUARY}\n{'='*60}")

    MOUTH_LAT, MOUTH_LON = _cfg['mouth']
    SHP_NAME = _cfg.get('shp_name', ACTIVE_ESTUARY)
    ...
    # ── Part 2: spatial maps ─────────────────────────────────────────────────
    yearly_map = yearly_col.filter(ee.Filter.inList("year", CLASSIFICATION_YEARS))
    mode_class = yearly_map.select("waterClass").mode().clip(aoi).rename("waterClass")
    ...
    wc_map = array["waterClass"]

    gsw_composite = ee.Image("JRC/GSW1_4/GlobalSurfaceWater").clip(aoi)
    ...
    im_seas = axes[1].imshow(
        seas_masked, cmap="Blues", vmin=1, vmax=12, extent=extent, origin="upper",
    )
    divider2 = make_axes_locatable(axes[1])
    cax2 = divider2.append_axes("right", size="3%", pad=0.1)
    cb2 = fig.colorbar(im_seas, cax=cax2)
    ...
    im_rec = axes[2].imshow(
        rec_masked, cmap=cmap_rec, vmin=1, vmax=100, extent=extent, origin="upper",
    )
    divider3 = make_axes_locatable(axes[2])
    cax3 = divider3.append_axes("right", size="3%", pad=0.1)
    cb3 = fig.colorbar(im_rec, cax=cax3)
    ...
    im_chg = axes[3].imshow(
        change_masked, cmap="RdBu", vmin=-100, vmax=100, extent=extent, origin="upper",
    )
