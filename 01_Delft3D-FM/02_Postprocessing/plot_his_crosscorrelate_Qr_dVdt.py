...
Following Wang et al. (2025) but applied to hourly model output.
...
The tide is fully harmonic with a 12h period, so a 24h tidal averaging window
cancels it exactly (2 full cycles). No additional smoothing is applied.
...
and N_eff = N / TIDAL_WINDOW_HOURS (conservative for autocorrelated series).
...
TIDAL_WINDOW_HOURS = 24          # hours for tidal averaging (12h tide → 24h = 2 exact cycles)
...
for scenario_dir, data in scenario_data.items():
    ...
    # Tidal-smooth Q (24h window = 2 exact cycles of the 12h harmonic tide)
    Q_smooth = tidal_avg(Q_raw, window)

    print(f"Scenario {scenario_key} ({SCENARIO_LABELS.get(scenario_key, '')}): loaded OK")
...
# ── PLOT C: CCF validation ────────────────────────────────────────────────────
# Cross-correlation of tidal-averaged Q anomaly vs tidal-averaged dV/dt anomaly,
# following Wang et al. (2025) Eq. 3 but applied to hourly model output.
#
# The tide is fully harmonic (12h period); the 24h tidal average removes it
# exactly. No additional smoothing is applied beyond the tidal average.
#
# Significance threshold: |ρ| > 2/sqrt(N_eff)
# where N_eff = N / TIDAL_WINDOW_HOURS (conservative for autocorrelated series)
...
def compute_ccf(Q_river, dV_dt, max_lag_steps, min_peak_steps):
    """
    Normalised CCF (Wang et al. 2025, Eq. 3) on tidal-averaged series.
    The 12h harmonic tide is fully removed by the 24h tidal average applied
    during preprocessing — no additional smoothing is needed or applied here.

    Returns lags_days, ccf_vals, ccf_lag (days or None), ccf_sign, sig_thresh.
    """
    Q_anom  = Q_river - Q_river.mean()
    dV_anom = dV_dt   - dV_dt.mean()
    ...
    # Conservative N_eff: each tidal-averaging window is treated as one
    # independent observation (accounts for autocorrelation introduced by
    # the 24h moving average).
    N_eff      = max(n // TIDAL_WINDOW_HOURS, 3)
...
for scenario_key, scenario_name in CCF_SCENARIOS.items():
    ...
    fig.suptitle(
        ...
        f'(24h tidal avg only,  max lag {CCF_MAX_LAG_DAYS} d)',
        fontsize=13
    )
    ...
    for i, (box_start, box_end) in enumerate(boxes):
        ...
        if ccf_lag is not None:
            ax.axvline(ccf_lag, color=d['color'], lw=1.5, ls=':',
                       label=f'Δt={ccf_lag:.1f}d ({ccf_sign})')
...
print("\nAll CCF plots saved to:", output_dir)


# ── PLOT D: SLIDING WINDOW CCF ────────────────────────────────────────────────
# For each section and scenario, compute the CCF in a moving window across the
# full simulation. At each window position, extract the dominant lag (first
# significant local max of |rho|). Plot lag vs simulation time to reveal
# whether response time changes over the morphological evolution of the estuary.
#
# Output: one figure per scenario, sections across columns
...
print()
print("  Tidal averaging")
print(f"    Both signals smoothed with {TIDAL_WINDOW_HOURS}h centred moving average.")
print( "    Tide is fully harmonic with 12h period → 24h window removes it exactly.")
print( "    No additional smoothing is applied.")
print()
print("  Step 1 — Anomaly (mean removal)")
print( "    Q'(t)       = Q(t)      - mean(Q)")
print( "    (dV/dt)'(t) = dV/dt(t) - mean(dV/dt)")
print( "    Required by the Pearson correlation definition (Wang et al. Eq. 3).")
print()
print("  Step 2 — Normalised cross-correlation")
print()
...
print()
print("  Step 3 — Lag detection")
print( "    Delta_t = lag of first significant local maximum in |rho(tau)|")
print(f"    Significance threshold: |rho| > 2 / sqrt(N_eff)")
print(f"    N_eff = N / {TIDAL_WINDOW_HOURS}  (one independent obs per tidal avg window)")
print()
print("  Step 4 — Sign interpretation")
...
def sliding_ccf_lag(Q, dV_dt, window_steps, step_steps,
                    max_lag_steps, min_lag_steps):
    """
    Slide a window across the full timeseries. In each window:
      1. Remove mean (anomaly) — no extra smoothing beyond tidal avg
      2. Compute normalised CCF (Wang et al. Eq. 3)
      3. Find dominant lag = first significant local max of |rho|
         beyond min_lag_steps
      4. Record lag, sign, and peak |rho| as quality indicator
    ...
    for s in starts:
        ...
        # N_eff based on tidal averaging window
        N_eff      = max(window_steps // TIDAL_WINDOW_HOURS, 3)
        sig_thresh = 2.0 / np.sqrt(N_eff)

        # First significant local maximum in |rho|
...
for scenario_key, d in processed.items():
    ...
    fig.suptitle(
        ...
        f"max lag={SW_MAX_LAG_DAYS}d, 24h tidal avg only)",
        fontsize=13
    )
    ...
    for i, (box_start, box_end) in enumerate(boxes):
        ...
        N_eff      = max(sw_window_steps // TIDAL_WINDOW_HOURS, 3)
        sig_thresh = 2.0 / np.sqrt(N_eff)
...
#%%
# """
# Cross-correlation function-based lag analysis: response time of estuarine sections to river discharge variability.

# Following Wang et al. (2025) but applied to hourly model output.

# METHOD — Normalised cross-correlation (static cross-correlation function)
# ───────────────────────────────────────────────────
# For tidal-averaged Q_river(t) and dV/dt(t) of length N, the normalised CCF is:
...
#     """
#     Q_s  = tidal_avg(Q_river)
#     dV_s = tidal_avg(dV_dt)
