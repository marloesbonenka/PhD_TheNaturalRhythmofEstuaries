...
# %% ============================================================
#    PLOT 5b — RECOVERY TIME  (cross-correlation Q_river → dV/dt)
#    One figure per km section, all scenarios overlaid
# ===============================================================
for (box_start, box_end) in boxes:
    ...
    for scenario_key, d in processed.items():
        ...
        ax.plot(lags_days, corr_plot, linewidth=2, color=d['color'],
                label=f"{d['label']}  (peak: {peak_lag:.1f} d)")

    ax.axhline(0, color='black', lw=0.8, ls='--')
    ax.set_xlabel('Lag (days)', fontsize=11)
    ax.set_ylabel('Normalised cross-correlation (–)', fontsize=11)
    ax.set_title(f'Recovery time  Q_river → dV/dt — section {box_start}–{box_end} km',
                 fontsize=12)
    ax.legend(title='Scenario', fontsize=9)
    ax.grid(alpha=0.25)
    fig.tight_layout()
    fig.savefig(
        output_dir / f"section_{box_start}-{box_end}km_recovery_allscenarios.png",
        dpi=300, bbox_inches='tight'
    )
    plt.show()
