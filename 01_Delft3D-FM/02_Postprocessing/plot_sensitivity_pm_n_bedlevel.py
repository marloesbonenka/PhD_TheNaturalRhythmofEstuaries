...
# env_min / env_max are width-averaged bed level [m], matching _get_y().
        ...
        for _subfolder in NOISY_SUBFOLDERS:
            ...
            for _tdt, _ts_idx, _adt in _snaps_n:
                ...
                for _k in range(len(_x_bins) - 1):
                    ...
                    _bin_means.append(
                        np.mean(_bl[_bm]) if np.any(_bm) else np.nan
                    )

                _noisy_profiles.setdefault(_snap_key, []).append(np.array(_bin_means))
