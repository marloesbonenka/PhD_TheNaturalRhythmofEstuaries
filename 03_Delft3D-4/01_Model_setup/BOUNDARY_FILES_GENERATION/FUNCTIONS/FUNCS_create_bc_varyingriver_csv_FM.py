...
def generate_river_discharges_fm(grid_info, params, output_dir, start_date_str='2024-01-01 00:00:00'):
    ...
    # #The below allocation causes errors/instabilities in Delft3D-FM: 
    # # 3. Dynamic Allocation Weights
    # rng_w = np.random.default_rng(0)
    # weights_stack = np.zeros((num_cells, num_steps))
    # for cell_idx in range(num_cells):
    #     base = np.zeros(num_steps)
    #     for i in range(4):
    #         period = rng_w.integers(30, 120) * 24 * 60 / time_step
    #         base += rng_w.uniform(0.7, 1.0) * np.sin(2 * np.pi * np.arange(num_steps) / period + rng_w.uniform(0, 2 * np.pi))
    #     weights_stack[cell_idx, :] = base
    
    # weights_stack -= np.min(weights_stack)
    # weights_stack += 0.5
    # weights_stack /= weights_stack.sum(axis=0)
