...
def plot_map(x, y, data, param_type, col_indices, N_coords, timestep, scenario, save_dir, save_figure=False):
    ...
    # Plot bed level
    mesh = ax.pcolormesh(x, y, plot_data, shading='auto', cmap=terrain_like, norm=norm)
    cbar = fig.colorbar(mesh, ax=ax, shrink=0.5, orientation='vertical', 
                       label=label)
    ...
    if save_figure:
        figname = os.path.join(save_dir, f'map_{param_type}_at_timestep_{timestep}.png')
...
def plot_velocity(x, y, data, velocity_type, col_indices, N_coords, timestep, scenario, save_dir, save_figure=False):
    ...
    cbar = fig.colorbar(mesh, ax=ax, shrink=0.5, orientation='vertical', label=label)
